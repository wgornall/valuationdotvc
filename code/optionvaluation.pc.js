	...
	var dataDate;
		...
		function loadCoData() {
			...
			if	(CompanyIndex < 0 ) {	
				...
				document.getElementById("in-Ticker").value = uTicker[CompanyIndex];	
				callFinancialAPI();
		...
		function callFinancialAPI() {
			...
			request.open('GET', 'https://finnhub.io/api/v1/stock/candle?symbol='+uTicker[CompanyIndex]+'&adjusted=true&resolution=D&count=100&token=bpducrnrh5rauiikkl40', true);
			...
			request.onload = function() {
				...
				monthlyPriceHistory = data['c'];
				dataDate = data['t'];
				
				document.getElementById("in-SharePrice-early").value = Math.round(100*monthlyPriceHistory[monthlyPriceHistory.length-1])/100;		 
			    callFinancialAPI2()
			}
			request.send(); 
		...
		function callFinancialAPI2() { 
			var request = new XMLHttpRequest();
			request.open('GET', 'https://finnhub.io/api/v1/stock/candle?symbol=SPY&adjusted=true&resolution=D&count=100&token=bpducrnrh5rauiikkl40', true);
			// if you steal my API key, you are a meanie.
 
			request.onload = function() {
				var data = JSON.parse(this.response);  
				
				monthlyPriceHistoryIndex = data['c'];
		...
		function callFinancialAPI3() { 
			...
			dataDate = dataDate[dataDate.length-1];
			...
			var volatility = Math.sqrt(251*(monthlyRetSumOfSq - monthlyRetSum*monthlyRetSum/monthlyVolatilityMonths)/(monthlyVolatilityMonths-1));
			document.getElementById("in-Volatility").value = Math.round(100* volatility*10)/10;
			
			var indexVolatility = Math.sqrt(251*(monthlyIndexRetSumOfSq - monthlyIndexRetSum*monthlyIndexRetSum/monthlyVolatilityMonths)/(monthlyVolatilityMonths-1));
			...
			document.getElementById("in-Beta").value = Math.round(beta*100)/100;
				...
				document.getElementById("DataFoundCo").innerHTML = "Data for "+trialCoName+" as of "+Date(dataDate*1000)+" loaded.";
...
function updatevisibleinputs() {
	if(  document.getElementsByName("in-ShareType")[1].checked ) {
		...
		document.getElementById("op-oors1").innerHTML = 'options';
		document.getElementById("op-oors2").innerHTML = 'options';
		...
		document.getElementById("op-oors1").innerHTML = 'shares';
		document.getElementById("op-oors2").innerHTML = 'shares';
